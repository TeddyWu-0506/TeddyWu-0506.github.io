...
- **裁定（2026-09-22）**：站内数字错了，**100 万+ 为准**。`facts.json` 的 `scale.spend` 改为 `100W+`，`facts_rules.py` 的识别式同步改为 `r'100W\+'`（不同步的话，将来 HTML 里裸写 `100W+` 就查不出来了）。PDF 不动。
- **执行时顺带修掉**：`--check` 汇总这一行用 `out.strip()` 后做子串计数，把首行的缩进吃掉了，于是 5 条核对只报 4 条——覆盖率被少报一项。改成按 lstrip 后的行首匹配计数。
- **状态**：☑ 批次 E（数据裁定）
